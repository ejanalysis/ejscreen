...
#'
#' @description
#'   Download EJSCREEN dataset from FTP site, and import to R as data.table, 
...
#' @export
ejscreen.download <- function(folder=getwd(), yr=2017, ftpurl, zipname, csvname, addflag=FALSE, cutoff=80, or.tied=TRUE) {
  ...
  calendaryear <- as.numeric(format(Sys.time(), "%Y"))  # as.numeric(substr(date(), 21, 24)) # So far the version year has been same as? prior to current actual year? 2017 version was available in 2017?
  if (yr == calendaryear) warning('This version might not be available yet.')
  if (!(yr %in% 2015:calendaryear)) {stop('Invalid year')}
  
  ftpurl <- paste('ftp://newftp.epa.gov/EJSCREEN/', yr, sep = '') # coerces yr to character. Hopefully will keep this format
  ...
  zipcsvnames <- function(yr) {
    ...
    if (yr > 2017) {
      zipname <- '' 
      csvname <- paste('EJSCREEN_', yr, '_USPR_Public.csv', sep = '') # hopefully will keep this format
    }
  ...
  setwd(folder)
  if (zipname == '') {myfilename <- csvname} else {myfilename <- zipname}
  mypathfile <- file.path(ftpurl, myfilename)
  
  #'#############################
  # DOWNLOAD from the FTP site
  #'#############################
  cat('Attempting to download dataset from ', mypathfile, 'and saving in', folder, ' \n')
  cat('This normally takes a few minutes. \n')
  x <- download.file(mypathfile, myfilename)
  if (x != 0) {stop('Download failed.')}
  if (!(file.exists(myfilename))) {stop('download attempted but saved zip file not found locally')}
  ...
  if (!(file.exists(csvname))) {stop('csv file should have been downloaded but not found in folder where expected')}
  ...
  if (addflag) {
    ...
    x <- ejanalysis::flagged(bg[ , names.ej.pctile], cutoff = cutoff, or.tied = or.tied)
    bg$flagged <- x
