...
#' @param zonecols Optional. Must set to NULL if no zones wanted, because default is c('ST', 'REGION'), names of cols in x that contain zone codes, such as State names or Region numbers, used to create a lookup table file for each of the zonecols, with separate percentiles calculated within each zone.
...
#' @param cols Optional vector of colnames of x that need percentile lookup tables, or 'all' which means all numeric fields in x.
...
#'  }
#' @export
ejscreen.lookuptables <- function(x, weights=x$pop, cols, zonecols=c('ST', 'REGION'), folder=getwd(), missingcode=NA) {

  ################################
  #	NOTE THESE FUNCTIONS ARE WRITTEN ASSUMING THE DATA IS IN A DATAFRAME CALLED places <- x (more generic code that requires passing the data.frame is elsewhere)
  ################################
  ...
  if (missing(cols)) {
    ...
    cols <- analyze.stuff::change.fieldnames(cols, oldnames = ejscreenformulas$gdbfieldname, newnames = ejscreenformulas$Rfieldname)
  }
  if (cols[1]=='all') {cols <- names(places)[sapply(places, is.numeric)]}

  # *** Written assuming places data.frame is in memory in current environment,
  # not passed to functions here
  if (any(!(cols %in% names(places)))) {stop('Not all cols are found among colnames of x')}
  ...
  table.pop.pctile	<- function(x,wt) {
    Hmisc::wtd.quantile(x, weights=wt, probs=seq(0,1,by=0.01), type='i/n', na.rm=TRUE )
  }
  ...
  x <- write.wtd.pctiles(column.names=cols, wts=weights, folder=folder, filename='output', missingcode=missingcode)

  for (i in 1:length(zonecols)) {
    y <- write.wtd.pctiles.by.zone(column.names=cols, wts=weights, folder=folder,
                                   filename=paste('out', zonecols[i], sep='_'), zone.vector=places[ , zonecols[i]], missingcode=missingcode)
  }
