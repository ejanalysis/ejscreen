if (1==0) {
  ...
  formulastoexcel <- function(x=ejscreenformulas, filename='excelformulas.csv', rangenames=FALSE, maxrow=250000, folder=getwd(), wtscolname = 'pop') {
    ...
    exceletters <- expand.grid(LETTERS[1:26], LETTERS[1:26], stringsAsFactors = FALSE)
    exceletters <- paste(exceletters[,2], exceletters[,1], sep='')
    exceletters <- c(LETTERS, exceletters)
    exceletters <- exceletters[1:NROW(x)]  # so that a logical index as long as x rows but shorter than exceletters will not be recycled
    ###########################################
    ...
    exrow <- 2 + NCOL(x) # added 1+1 because it is the next available row  and header row gets added to excel after t()
    ...
    ls.Rfieldname <- x$Rfieldname[order(nchar(x$Rfieldname), decreasing = TRUE)]
    ls.cellref <- cellref[order(nchar(x$Rfieldname), decreasing = TRUE)]

    exform <- x$formula
    exform[is.na(x$formula)] <- 0 # but gets wiped out by character formulas, NA

    ####################################
    for (i in 1:NROW(x)) {exform <- gsub(ls.Rfieldname[i], ls.cellref[i], exform)}
    ####################################
    ...
    x$excelformula[isumform] <- gsub('sum\\(([A-Z]*)([0-9]*)\\)', 'SUM($\\1$\\2:$\\1$250000)', x$excelformula[isumform])
    ...
    excel.wtd.pctile <- function(excol='A', wtscol='B', exrow=1, maxrow=250000, rangenames=FALSE) {
      if (!rangenames) {
        results = paste('=SUMIF(',
                        '$', excol, '$', exrow, ':$', excol, '$', maxrow,
                        ', "<"&', excol, exrow, ', ',
                        '$', wtscol, '$', exrow, ':$', wtscol, '$', maxrow,
                        ') / SUM(',
                        '$', wtscol, '$', exrow, ':$', wtscol, '$', maxrow,
                        ')',
                        sep='')
    ...
    is.pctilecol <- grepl('^pctile.', gsub('^pctile.text', '', x$Rfieldname))
    ...
    if (rangenames) {
      pctile.forms <- excel.wtd.pctile(excol=basecolname, wtscol = wtscol, rangenames=rangenames, maxrow = maxrow)
    } else {
      pctile.forms <- excel.wtd.pctile(excol=basecoletter, wtscol = wtscol, exrow = exrow, rangenames=rangenames)
    }
    ...
    excel.bin <- function(excol='A', exrow=1, rangenames=FALSE) {
    ...
    if (rangenames) {
      bin.forms <- excel.bin(excol=basecolname, exrow = exrow, rangenames=TRUE)
    } else {
      bin.forms <- excel.bin(excol=basecoletter, exrow = exrow, rangenames=FALSE)
    }
    ...
    write.csv(y, row.names = FALSE, file=filename, folder=folder)
