...
#' @export
ejscreen.rollup.save <- function(myrollup, myfolder = getwd(), filenamebase = 'EJSCREEN', scalename = c('tracts'), filename.R, filename.csv, save.R = TRUE, save.csv = TRUE) {
  # helper function used by ejscreen.rollup.all
  # Saves just for one scale - loop outside this for all
  if (!save.csv & !save.R) {warning('Why would you want to call ejscreen.rollup.save but not actually save any file?')}
  
  # construct filenames if missing & needed 
  if (save.csv & missing(filename.csv)) {
    filename.csv <- file.path(myfolder, paste(filenamebase, scalename, 'data.csv'))
  }
  if (save.R & missing(filename.R)) {
    filename.R <- file.path(myfolder, paste(filenamebase, scalename, 'data.RData'))
  }
  ...
  #  } # no longer need loop
  output <- c(
    ifelse(save.R, file.path(myfolder, filename), NA),
