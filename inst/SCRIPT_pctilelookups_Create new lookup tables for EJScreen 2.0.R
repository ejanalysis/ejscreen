...
# ejanalysis::make.bin.pctile.cols.byzone		May be obsolete- Percentile and bin number fields, by zone, such as percentiles within each State
...
# ejscreen::lookupRegions		The EPA-Region-level latest version of the EJSCREEN percentile lookup table.
# ejscreen::lookupStates		The State-level latest version of the EJSCREEN percentile lookup table.
# ejscreen::lookupUSA		The nationwide most recent version of the EJSCREEN percentile lookup table.
...
# zone.vector = bg$ST[bg$ST %in% c('NY','DE')], filename = 'lookupStatesTEST.csv')
  ...
  lookupStates <-  ejanalysis::write.wtd.pctiles.by.zone(
    ...
    wts = bg$pop, 
    zone.vector = bg$ST, 
    filename =  'lookupStates21plusDsub_noPR')
